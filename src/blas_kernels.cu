...
/*
__global__ void reorg_kernel(int N, float *x, int w, int h, int d, int c, int batch, int stride, int forward, float *out)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i >= N) return;
    int in_index = i;
    ...
    int out_c = c/(stride*stride*stride);
    ...
    //printf("%d\n", offset);
    int out_index = w2 + w*stride*(h2 + h*stride*(d2 + d*stride*(c2 + out_c*b)));
...
/*
extern "C" void reorg_ongpu(float *x, int w, int h, int d, int c, int batch, int stride, int forward, float *out)
{
    int size = w*h*d*c*batch;
    reorg_kernel<<<cuda_gridsize(size), BLOCK>>>(size, x, w, h, d, c, batch, stride, forward, out);
    check_error(cudaPeekAtLastError());
}
*/
